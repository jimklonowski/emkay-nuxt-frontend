...
export default {
  ...
  watch: {
    async 'config.report_type' () {
      ...
      await this.loadColumns(this.config.report_type)
    },
    ...
    async step () {
      if (this.step === 5) {
        ...
        try {
          ...
          await this.fetchReport(this.config)
        } catch (error) {
  ...
  methods: {
    ...mapActions({
      loadColumns: 'my-reports/fetchColumnGroups',
      fetchReport: 'my-reports/fetchData'
    }),
