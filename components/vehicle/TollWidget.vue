...
export default {
  ...
  computed: {
    ...
    tollRoute: vm => vm.localePath({ path: `/vehicle/${vm.vehicle_number}/toll`, query: { start: vm.start_date, end: vm.end_date } })
  },
  watch: {
    /**
  ...
  methods: {
    async populateWidget () {
      await this.$store.dispatch('vehicle/fetchTollHistory', {
