...
export default {
  ...
  computed: {
    ...
    violationsRoute: vm => vm.localePath({ path: `/vehicle/${vm.vehicle_number}/violations`, query: { start: vm.start_date, end: vm.end_date } })
  },
  watch: {
    /**
  ...
  methods: {
    async populateWidget () {
      await this.$store.dispatch('vehicle/fetchViolationHistory', {
